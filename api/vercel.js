    ...
    return {
        list: t?.list?.map(e => {
            let o = [{
                    ...
                    label: "\u64AD\u653E\u91CF",
                    value: e?.stat?.view && m(e.stat.view)
                }, {
                    label: "\u8FFD\u756A\u6570",
                    value: e?.stat?.follow && m(e.stat.follow)
                }, {
                    label: "\u6295\u5E01\u6570",
                    value: e?.stat?.coin && m(e.stat.coin)
                }, {
                    label: "\u5F39\u5E55\u6570",
...
var F = "edge",
    G = ["hkg1", "hnd1", "kix1", "sin1"];
async function H(t) {
    let a = new URL(t.url),
        e = S(f(a));
    if (isMock) {
        // 修改1：先判断 bgm（bangumi），再判断 bilibili
        if (a.pathname.endsWith("bgm")) return Response.json(mockDataMap.bgm);
        if (a.pathname.endsWith("bilibili")) return Response.json(mockDataMap.bilibili);
    }
    let o = {};
    try {
        o = customData
    } catch {}
    // 修改2：先处理 bgm（bangumi）请求，再处理 bilibili 请求
    return a.pathname.endsWith("bgm") ? await y(e, process.env) : a.pathname.endsWith("bilibili") ? await h(e, process.env) : a.pathname.endsWith("custom") ? d(e, o) : Response.json({
        code: 404,
        message: "not found",
        data: {}
    }, {
...
export {
    H as GET, G as preferredRegion, F as runtime
};
